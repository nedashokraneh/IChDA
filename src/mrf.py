...
class MarkovRandomField:
    def __init__(self, n, edges, obs, out_dir, n_state=10, n_iter=10, tol=1e-2, lbp_iter=100):
        ...
        self.edges_matrix = csr_matrix((edges[:, 2], (edges[:, 0], edges[:, 1])), shape=(n, n))
    ...
    def init_gmm(self):
        ...
        gmm_model = mixture.GaussianMixture(n_components=self.n_state, covariance_type='full')
    ...
    def init_trans(self):
        ...
        #self.edge_potential = mrf_util.log_transform(trans_matrix.toarray())
    ...
    def loop_belief_propagation(self, max_iter=100):
        ...
        message = csr_matrix(mrf_util.log_transform(np.ones((self.n_state,self.n_edges))))
        ...
        #print("lbp.")
        mrf_util.print_log(time.ctime() + " loopy belief propagation.", self.out_dir + "/log.txt")
        self.last = []
        ...
        for i in range(max_iter):
            ...
            #normalization
            belief = csr_matrix(belief_new - mrf_util.sparse_logsumexp_row(belief_new))
            ...
            if self.check_converge():
                break
                #pass
        mrf_util.print_log(time.ctime() + " Number of iteration: " + str(i), self.out_dir + "/log.txt")
    ...
    def solve(self, max_iter=10):
        ...
        for iter_i in range(max_iter):
            ...
            for i in range(self.n_state):
                #self.gaussian[:,i] = mrf_util.get_multi_normal_pdf(gmm_model.means_[i,:],gmm_model.covariances_[i,:,:], self.obs)
                data = self.obs[estimate_label==i]
                if len(data)>0:
                    self.gaussian[:,i] = mrf_util.get_multi_normal_pdf(np.mean(data, axis=0),np.cov(data, rowvar=0), self.obs)
            ...
            trans_matrix = trans_matrix - logsumexp(trans_matrix)
            self.edge_potential = trans_matrix
            self.iters_labels.append(estimate_label)
